  ...
  generatePassword() {
    ...
    if (excludeChars) {
      for (let char of excludeChars) {
        for (let set in charSets) {
          charSets[set] = charSets[set].replace(
    ...
    if (requireAll) {
      ...
      if (uppercase && charSets.uppercase)
        required.push(this.getRandomChar(charSets.uppercase));
      if (lowercase && charSets.lowercase)
        required.push(this.getRandomChar(charSets.lowercase));
      if (numbers && charSets.numbers)
        required.push(this.getRandomChar(charSets.numbers));
      if (symbols && charSets.symbols)
        required.push(this.getRandomChar(charSets.symbols));
  ...
  setupEventListeners() {
    ...
    document.getElementById("generateBtn").addEventListener("click", () => {
      ...
      if (password) {
        ...
        document.querySelector(".strength-meter-bar").style.width =
          strength.score + "%";
        document.querySelector(".strength-meter-bar").style.backgroundPosition =
          `${100 - strength.score}% 0`;
      }
    });
    // Copy button
